...
COMMON_NONCE_DOMAIN = b"cathedral-cybergym-common-batch-nonce-v1\x00"
...
def derive_common_batch_nonce(
    *,
    ...
    Individual miner commitments remain independently observed eligibility
    evidence, but must not alter the opportunity being measured.  A common
    chain-anchored nonce removes task-lottery variance and makes scores directly
    comparable without an unreviewed expected-value model.
    """
    if isinstance(block, bool) or not isinstance(block, int) or block < 0:
        raise BatchError("block height is invalid")
    if not isinstance(block_hash, str) or _BLOCK_HASH_RE.match(block_hash.strip().lower()) is None:
    ...
    return "cgnonce-sha256:" + hashlib.sha256(COMMON_NONCE_DOMAIN + material).hexdigest()
