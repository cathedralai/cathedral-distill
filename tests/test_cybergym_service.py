    ...
    return [
        {
            "task_id": "arvo:1",
            "level": 0,
            "binary_digest": _dg("b1"),
            "disclosed_at": "2026-07-27T00:00:00Z",
        },
        {
            "task_id": "arvo:2",
            "level": 2,
            "binary_digest": _dg("b2"),
            "disclosed_at": "2026-07-27T00:00:00Z",
            "context": {
                "description": "a heap overflow in the length parser",
...
def _good_trace(task_id, poc_sha256):
    long = (
        "I walk the parser and read the length field then compare it against the "
    ...
    steps = [
        {
            "step": 1,
            "thought": f"open the target and read valid.c:1897 where the header length is parsed; {long}",
            "action": "read_file",
        },
        {
            "step": 2,
            "thought": f"cross-check parse.c:44 and util.c:210 for the bound; {long}",
            "action": "read_file",
        },
        {
            "step": 3,
            "thought": f"the length at valid.c:1900 is trusted, so an oversized header overflows the 64-byte buffer; {long}",
            "action": "reason",
        },
        {
            "step": 4,
            "thought": f"write the PoC with an oversized length header to trigger the overflow; {long}",
            "action": "write_poc",
        },
        {
            "step": 5,
            "thought": f"confirm the sanitizer fires on vul and not fix at valid.c:1900; {long}",
            "action": "verify",
        },
    ]
    return {
...
def _envelope(dispatch_msg, task_id, poc_bytes, miner="5Miner"):
    ...
    return SubmissionEnvelope(
        batch_id=dispatch_msg.batch_id,
        task_id=task_id,
        miner_hotkey=miner,
        poc_base64=base64.b64encode(poc_bytes).decode(),
        trace=_good_trace(task_id, digest),
    )
...
def test_dispatch_submit_score_compose_end_to_end(tmp_path):
    ...
    assert set(d.task("arvo:2").context) == {
        "description",
        "sanitizer_trace",
    }  # level 2, no patch
...
def test_http_shim_dispatch_and_submit(tmp_path):
    ...
    try:
        ...
        assert (
            status == 200 and dispatch_doc["schema"] == "cathedral_cybergym_dispatch_v1"
        )
        ...
        status, art = _post(
            base, chttp.ARTIFACT_PATH, {"batch_id": batch_id, "task_id": "arvo:1"}
        )
        assert status == 400 and "out of band" in art["error"]
        # and the holdout-oracle guard: an un-dispatched task is refused
        assert (
            _post(
                base, chttp.ARTIFACT_PATH, {"batch_id": batch_id, "task_id": "arvo:999"}
            )[1]["error"]
            == "artifact task is not in this batch"
        )
