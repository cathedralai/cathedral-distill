...
def test_wire_messages_are_byte_identical_to_the_backend():
    ...
    for args in [("5M", "sha256:aa", 7), ("5M", "sha256:aa", 7, "https://api.openai.com/v1", "gpt-5")]:
...
def test_declared_model_is_read_from_the_agent_zip_env():
    raw = _zip({"agent.py": b"# solver\n",
                ".env": b'AGENT_MODEL=deepseek-v4-pro\nAGENT_BASE_URL="https://api.deepseek.com/v1"\n'})
    assert mc._model_from_zip(raw) == ("https://api.deepseek.com/v1", "deepseek-v4-pro")
    assert mc._model_from_zip(_zip({"agent.py": b"x"})) == ("", "")   # no .env → nothing declared
